...
void PPU::RenderScanline()
{
    ...
    for (uint8_t pixel_idx = 0; pixel_idx < SCREEN_WIDTH; pixel_idx++)
        this->framebuffer[this->scanline * SCREEN_WIDTH + pixel_idx] = this->scanline_pixels[pixel_idx].color_idx;
}
...
void PPU::RenderBackground()
{
    ...
    for (int x = 0; x < SCREEN_WIDTH; x++)
    {
        ...
        const uint8_t tile_data_1 = this->memory->Read8(tile_addr + pixel_y * 2);
        const uint8_t tile_data_2 = this->memory->Read8(tile_addr + pixel_y * 2 + 1);
...
void PPU::RenderObjects()
{
    ...
    for (int8_t obj_idx = this->objects.size() - 1; obj_idx >= 0; obj_idx--)
    {
        ...
        const bool flip_y = object.attributes & OBJ_FLIP_Y;
        const bool flip_x = object.attributes & OBJ_FLIP_X;
        const bool has_priority = object.attributes & OBJ_PRIORITY;
        const uint8_t palette_addr = (object.attributes & OBJ_PALETTE) ? IO_ADDR_OBP1 : IO_ADDR_OBP0;
        const uint8_t palette = this->memory->ReadIO(palette_addr);

        uint8_t tile_index = object.tile_index;
        ...
        const uint8_t tile_data_1 = this->memory->Read8(tile_addr + tile_line * 2);
        const uint8_t tile_data_2 = this->memory->Read8(tile_addr + tile_line * 2 + 1);
        ...
        for (int px = 0; px < 8; px++)
        {
            ...
            this->scanline_pixels[screen_x].color_idx = color;
        }
    }
}